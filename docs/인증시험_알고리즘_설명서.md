# 선박용 냉각 시스템 인증 시험 알고리즘 설명서

**문서 버전**: 1.0
**작성일**: 2025-11-28
**대상 시스템**: PLC Simulator + Edge Computer + HMI 통합 시스템

---

## 1. 시험 개요

본 문서는 선박용 냉각 시스템의 인증 시험에 사용되는 3가지 테스트의 알고리즘과 실행 방법을 설명합니다.

### 1.1 시험 목록

| 시험 번호 | 시험명 | 측정 항목 | 합격 기준 |
|-----------|--------|-----------|-----------|
...
AI 제어기가 계산을 완료한 시점부터 PLC를 통해 VFD(Variable Frequency Drive) 제어 명령이 실행 완료되기까지의 시간을 측정합니다.
...
**필요 프로세스**: PLC Simulator만 실행 (Edge Computer 모듈은 테스트에서 직접 import)
...
온도 변화에 따라 AI 제어기가 올바른 방향으로 주파수를 조절하는지 검증합니다.
...
#### 3.3.1 SWP (Sea Water Pump) 제어

```
제어 센서: T5 (Cooler FW Outlet)
목표: FW 출구 온도 유지
...
#### 3.3.3 FAN (Engine Room Fan) 제어

```
제어 센서: T6 (Engine Room)
목표: 기관실 온도 43°C 유지
...
**필요 프로세스**: 없음 (AI 제어 모듈만 import하여 순수 계산 로직 검증)
...
PLC의 센서값이 변경된 후 HMI 화면에 반영되기까지의 시간을 측정합니다.
...
```python
def wait_for_value(self, sensor_key, expected_value, timeout, tolerance):
    """
    ...
    while (time.time() - start_wait) < timeout:
        if sensor_key in self.latest_sensors:
            ...
            if abs(sensor_val - expected_value) < tolerance:
                return self.latest_time  # 수신 시간 반환
...
**필요 프로세스**: PLC Simulator + HMI Backend

---

## 5. 시험별 필요 프로세스 요약

| 시험 | PLC Simulator | Edge Computer | HMI Backend |
|------|:-------------:|:-------------:|:-----------:|
...
```
Edge_Computer_V1/test_results/
├── test_results_plc_response_YYYYMMDD_HHMMSS.csv  (상세 결과)
...
```
Edge_Computer_V1/test_results/
├── test_results_ai_prediction_YYYYMMDD_HHMMSS.csv  (상세 결과)
└── test_summary_ai_prediction_YYYYMMDD_HHMMSS.csv  (통계 요약)
```
...
```
HMI_V1/test_results/
├── test_results_hmi_refresh_YYYYMMDD_HHMMSS.csv  (상세 결과)
